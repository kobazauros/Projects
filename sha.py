...
def to_binary(input_str, conv_type='string'):
    ...
    conv_type = 'string' - string is converted as a
                string even if it contains digits
    conv_type = 'digits' - if string contains only
                digits it is converted to binary as an integer
    """
    if conv_type == 'string':
        try:
...
def logical_add(a_str,
    ...
    """
    res = int(a_str, 2) + int(b_str, 2) + \
        int(c_str, 2) + int(d_str, 2) + int(e_str, 2)
...
for i in message_chunks:
    ...
    for j in range(16, len(word_32bit)):
        ...
        word_32bit[j] = logical_add(
            word_32bit[j - 16], s0, word_32bit[j - 7], s1)
        word_32bit[j] = word_32bit[j].zfill(32)

    a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7

    for i in range(0, 64):
        S1 = logical_xor(rightrotate(e, 6), rightrotate(
            e, 11), rightrotate(e, 25))
        ch = (int(e, 2) & int(f, 2)) ^ ((~int(e, 2)) & int(g, 2))
        ch = f"{ch:b}".zfill(32)
        temp1 = logical_add(h, S1, ch, round_constants[i], word_32bit[i])
        S0 = logical_xor(rightrotate(a, 2), rightrotate(
            a, 13), rightrotate(a, 22))
        maj = (int(a, 2) & int(b, 2)) ^ (
            int(a, 2) & int(c, 2)) ^ (int(b, 2) & int(c, 2))
